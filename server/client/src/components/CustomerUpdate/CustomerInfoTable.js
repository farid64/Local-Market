...
class ReactVirtualizedTable extends React.Component {
  ...
  actuallyLoadMore() {
    // fake new data
    let newItems = [];
    // let s = this.state.sortedList.length + 1;
    for (let i = 0, l = 10; i < l; i++) {
      // newItems.push
      const randomSelection = data[Math.floor(Math.random() * data.length)];
      newItems.push(createData(...randomSelection));
    }
    this.setState({ sortedList: this.state.sortedList.concat(newItems) });
    // resolve the promise after data where fetched
    this.promiseResolve = this.promiseResolve();
    console.log(this.state.sortedList.length);
  }
}
